<?php
if( is_multisite() ) {
	function menu_multi_facebook_profile_admin_widgets(){
	// Create menu
	add_submenu_page( 'facebook-profile-master', 'Widgets', 'Widgets', 'manage_options', 'facebook-profile-master-admin-widgets', 'facebook_profile_master_admin_widgets' );
	}
}
else {
	// Create menu
	function menu_single_facebook_profile_admin_widgets(){
		if ( is_admin() )
		add_submenu_page( 'facebook-profile-master', 'Widgets', 'Widgets', 'manage_options', 'facebook-profile-master-admin-widgets', 'facebook_profile_master_admin_widgets' );
	}
}

function facebook_profile_master_admin_widgets(){
?>
<div class="wrap">
...
<h2><b>&nbsp;Widgets</b></h2>
<?php
if(!class_exists('facebook_profile_master_admin_widgets_table')){
	require_once( dirname( __FILE__ ) . '/facebook-profile-master-admin-widgets-table.php');
}
//Prepare Table of elements
$wp_list_table = new facebook_profile_master_admin_widgets_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'network_admin_menu', 'menu_multi_facebook_profile_admin_widgets' );
}
else {
add_action( 'admin_menu', 'menu_single_facebook_profile_admin_widgets' );
}
?>